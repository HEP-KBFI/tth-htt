...
//--- parse command-line arguments
  ...
  std::cout << "<analyze_Zctrl>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("analyze_Zctrl");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") ) 
    throw cms::Exception("analyze_Zctrl") 
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";

  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfg_analyze = cfg.getParameter<edm::ParameterSet>("analyze_Zctrl");
...
//--- declare histograms
  ...
  JetHistManager selBJet_looseHistManager_lead(makeHistManager_cfg(process_string, 
    "Zctrl/sel/leadBJet_loose", central_or_shift, 0));
  ...
  JetHistManager selBJet_looseHistManager_sublead(makeHistManager_cfg(process_string, 
    "Zctrl/sel/subleadBJet_loose", central_or_shift, 1));
  ...
  JetHistManager selBJet_mediumHistManager(makeHistManager_cfg(process_string, 
    "Zctrl/sel/BJets_medium", central_or_shift));
  selBJet_mediumHistManager.bookHistograms(fs);

  MEtHistManager selMEtHistManager(makeHistManager_cfg(process_string, 
    "Zctrl/sel/met", central_or_shift));
  selMEtHistManager.bookHistograms(fs);

  EvtHistManager_Zctrl selEvtHistManager(makeHistManager_cfg(process_string, 
    "Zctrl/sel/evt", central_or_shift));
  selEvtHistManager.bookHistograms(fs);
  std::map<std::string, EvtHistManager_Zctrl*> selEvtHistManager_decayMode; // key = decay mode
  ...
  if ( isSignal ) {
    for ( vstring::const_iterator decayMode = decayModes_evt.begin();
      ...
      EvtHistManager_Zctrl* selEvtHistManager_ptr = new EvtHistManager_Zctrl(makeHistManager_cfg(decayMode->data(),
        "Zctrl/sel/evt", central_or_shift));
  ...
  if ( isMC ) {
    genEvtHistManager_beforeCuts = new GenEvtHistManager(makeHistManager_cfg(process_string, 
      "Zctrl/unbiased/genEvt", central_or_shift));
    genEvtHistManager_beforeCuts->bookHistograms(fs);
    genEvtHistManager_afterCuts = new GenEvtHistManager(makeHistManager_cfg(process_string, 
      "Zctrl/sel/genEvt", central_or_shift));
    genEvtHistManager_afterCuts->bookHistograms(fs);
    lheInfoHistManager = new LHEInfoHistManager(makeHistManager_cfg(process_string, 
      "Zctrl/sel/lheInfo", central_or_shift));
...
//--- apply preselection
    ...
    if ( (preselElectrons.size() == 2 &&                            !(selTrigger_1e || selTrigger_2e                                                        ) ) ||
	 (preselElectrons.size() == 1 && preselMuons.size() == 1 && !(selTrigger_1e ||                  selTrigger_1mu ||                   selTrigger_1e1mu) ) ||
	 (                               preselMuons.size() == 2 && !(                                  selTrigger_1mu || selTrigger_2mu                    ) ) ) {
      if ( run_lumi_eventSelector ) {
...
//--- apply final event selection 
    ...
    if ( (selElectrons.size() == 2 &&                         !(selTrigger_1e || selTrigger_2e                                                        ) ) ||
	 (selElectrons.size() == 1 && selMuons.size() == 1 && !(selTrigger_1e ||                  selTrigger_1mu ||                   selTrigger_1e1mu) ) ||
	 (                            selMuons.size() == 2 && !(                                  selTrigger_1mu || selTrigger_2mu                    ) ) ) {
      if ( run_lumi_eventSelector ) {
    ...
    int sumLeptonCharge = selLepton_lead->charge_ + selLepton_sublead->charge_;
    if ( sumLeptonCharge != 0 ) {
      if ( run_lumi_eventSelector ) {
	std::cout << "event FAILS lepton charge selection." << std::endl;
	std::cout << " (leading selLepton charge = " << selLepton_lead->charge_ 
		  << ", subleading selLepton charge = " << selLepton_sublead->charge_ << ")" << std::endl;
      }
      continue;
    }
