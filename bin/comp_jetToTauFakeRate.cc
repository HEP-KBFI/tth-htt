...
#include "tthAnalysis/HiggsToTauTau/interface/fitAuxFunctions.h" // EigenVector_and_Value, compEigenVectors_and_Values, fitFunction_and_legendEntry, makeControlPlot_graphs, makeControlPlot_fit
...
TGraphAsymmErrors* getGraph_jetToTauFakeRate(TH1* histogram_loose, TH1* histogram_tight, const std::string& graphName_jetToTauFakeRate)
{
  ...
  for ( int idxBin = 1; idxBin <= numBins; ++idxBin ) {
    ...
    compFakeRate(nPass, nPassErr, nFail, nFailErr, jetToTauFakeRate, jetToTauFakeRateErrUp, jetToTauFakeRateErrDown, errorFlag, true); // Capping uncert. to 0.01
    if ( errorFlag ) {
...
//--- read python configuration parameters
  ...
  const std::string yScale_str = cfg_comp.getParameter<std::string>("yScale");
  assert(yScale_str == "linear" || yScale_str == "log");
  const bool useYLogScale = yScale_str == "log";
