...
#include "tthAnalysis/HiggsToTauTau/interface/EvtHistManager_ZZctrl.h" // EvtHistManager_ZZctrl
...
//// so it's the same as the 4l region, but inverting the Z veto  and the 2b1B cut
...
/**
 * @brief Produce datacard and control plots for ZZctrl categories.
 */
...
//--- parse command-line arguments
  ...
  std::cout << "<analyze_ZZctrl>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("analyze_ZZctrl");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") )
    throw cms::Exception("analyze_ZZctrl")
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";

  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfg_analyze = cfg.getParameter<edm::ParameterSet>("analyze_ZZctrl");
  ...
  else throw cms::Exception("analyze_ZZctrl")
  ...
  switch(era)
  {
    ...
    default: throw cmsException("analyze_ZZctrl", __LINE__) << "Invalid era = " << era;
  }
...
//--- declare histograms
  ...
  const std::vector<std::string> cuts = {
    ...
    ">= 4 presel leptons",
    ">= N jets",
    ">= 2 loose b-jets || 1 medium b-jet inverted",
    "4 sel leptons",
    "HLT filter matching",
    "m(ll) > 12 GeV",
...
//--- apply preselection
  ...
  printCollection("preselLeptons", preselLeptonsFull);
    ...
    if ( (selBJets_loose.size() >= 2 || selBJets_medium.size() >= 1)) {
      if ( run_lumi_eventSelector ) {
        std::cout << "event " << eventInfo.str() << " FAILS selBJets inverted selection\n";
        printCollection("selJets", selJets);
    ...
    cutFlowTable.update(">= 2 loose b-jets || 1 medium b-jet inverted", evtWeightRecorder.get(central_or_shift_main));
...
//--- fill histograms with events passing final selection
    ...
    if(snm)
    {
      const double mT_lep1        = comp_MT_met(selLepton_lead->p4(), met.pt(), met.phi());
      const double mT_lep2        = comp_MT_met(selLepton_sublead->p4(), met.pt(), met.phi());
      const double mT_lep3        = comp_MT_met(selLepton_third->p4(), met.pt(), met.phi());
      const double mT_lep4        = comp_MT_met(selLepton_fourth->p4(), met.pt(), met.phi());
  ...
  for(const std::string & central_or_shift: central_or_shifts_local)
  {
    ...
    for(const leptonGenMatchEntry & leptonGenMatch_definition: leptonGenMatch_definitions)
    {
      ...
      const TH1* histogram_EventCounter = selHistManagers[central_or_shift][idxLepton]->evt_[default_cat_str]->getHistogram_EventCounter();
