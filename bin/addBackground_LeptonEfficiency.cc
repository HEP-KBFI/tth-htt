/** \executable addBackground_LeptonEfficiency
 *
 * Compute shape templates and normalization for fake lepton background to be used for LeptonEfficiency computation.
 *
...
//--- parse command-line arguments
  ...
  std::cout << "<addBackground_LeptonEfficiency>:" << std::endl;

//--- keep track of time it takes the macro to execute
  TBenchmark clock;
  clock.Start("addBackground_LeptonEfficiency");

//--- read python configuration parameters
  if ( !edm::readPSetsFrom(argv[1])->existsAs<edm::ParameterSet>("process") ) 
    throw cms::Exception("addBackground_LeptonEfficiency") 
      << "No ParameterSet 'process' found in configuration file = " << argv[1] << " !!\n";
  
  edm::ParameterSet cfg = edm::readPSetsFrom(argv[1])->getParameter<edm::ParameterSet>("process");

  edm::ParameterSet cfgAddBackground_LeptonEfficiency = cfg.getParameter<edm::ParameterSet>("addBackground_LeptonEfficiency");
  
  std::vector<categoryEntryType*> categories;
  edm::VParameterSet cfgCategories = cfgAddBackground_LeptonEfficiency.getParameter<edm::VParameterSet>("categories");
  ...
  std::string processData = cfgAddBackground_LeptonEfficiency.getParameter<std::string>("processData");
  std::string processLeptonFakes = cfgAddBackground_LeptonEfficiency.getParameter<std::string>("processLeptonFakes");
  vstring processesToSubtract = cfgAddBackground_LeptonEfficiency.getParameter<vstring>("processesToSubtract");

  vstring central_or_shifts = cfgAddBackground_LeptonEfficiency.getParameter<vstring>("sysShifts");
  ...
  clock.Show("addBackground_LeptonEfficiency");
