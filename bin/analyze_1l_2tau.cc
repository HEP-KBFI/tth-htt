...
//--- read python configuration parameters
  ...
  const bool do_tree = false; //(isSignal || process_string == "TTW" || process_string == "TTZ" || process_string == "TT") && !(central_or_shifts_local.size() > 1);
  ...
  switch(era)
  {
    ...
    default: throw cmsException("analyze_1l_2tau", __LINE__) << "Invalid era = " << static_cast<int>(era);
  }
  ...
  if ( era == Era::k2016 )
  {
    ...
    trigMatching_failsTrigger  = "failsTriggerMatchingLooseIso";
  ...
  else if ( era == Era::k2017 || era == Era::k2018 )
  {
    ...
    trigMatching_failsTrigger  = "failsTriggerMatchingLooseChargedIso";
    filterBit_passesTrigger = getTrigMatchingOption_2017and2018(trigMatching_passesTrigger);
  }
  else throw cmsException("analyze_1l_2tau", __LINE__) << "Invalid era = " << static_cast<int>(era);
...
//--- declare missing transverse energy
  ...
  struct selHistManagerType
  {
    ...
    std::map<std::string, std::map<std::string, EvtHistManager_1l_2tau*>> evt_htxs_;
    std::map<std::string, std::map<std::string, EvtHistManager_1l_2tau*>> evt_in_decayModes_;
    std::map<std::string, std::map<std::string, std::map<std::string, EvtHistManager_1l_2tau*>>> evt_htxs_in_decayModes_;
    std::map<std::string, EvtHistManager_1l_2tau*> evt_in_categories_;
    std::map<std::string, std::map<std::string, EvtHistManager_1l_2tau*>> evt_htxs_in_categories_;
    ...
    for (const GenMatchEntry * genMatchDefinition : genMatchDefinitions)
    {
      ...
      for(const std::string & evt_cat_str: evt_cat_strs)
      {
        ...
        for(const auto & kv: evt_htxs_binning)
        {
          ...
          selHistManager->evt_htxs_[evt_cat_str][kv.first] = new EvtHistManager_1l_2tau(makeHistManager_cfg(
	  ...
	  decayMode_and_genMatch += genMatchDefinition->getName();
          ...
          for(const std::string & evt_cat_str: evt_cat_strs)
          {
            ...
            for(const auto & kv: evt_htxs_binning)
            {
              ...
              selHistManager->evt_htxs_in_decayModes_[evt_cat_str][decayMode_evt][kv.first] = new EvtHistManager_1l_2tau(makeHistManager_cfg(
      ...
      for(const std::string & category: categories_evt)
      {
        ...
        for(const auto & kv: evt_htxs_binning)
        {
          ...
          selHistManager->evt_htxs_in_categories_[category][kv.first] = new EvtHistManager_1l_2tau(makeHistManager_cfg(htxs_process_and_genMatch,
            Form("%s/sel/evt", histogramDir_category.Data()), era_string, central_or_shift, "minimalHistograms"
          ));
...
//--- fill histograms with events passing final selection
    for(const std::string & central_or_shift: central_or_shifts_local)
    {
      ...
      for (const GenMatchEntry* genMatch : genMatches)
      {
        ...
        EvtHistManager_1l_2tau_Input fillVariables {
          selElectrons.size(),
          selMuons.size(),
          selHadTaus.size(),
          selJets.size(),
        ...
        {
          fillVariables.evtWeight = kv.second;
          ...
          for(const auto & kw: evt_htxs_binning)
          {
            ...
            {
              EvtHistManager_1l_2tau_Input fillVariables_htxs = fillVariables;
    ...
    if ( bdt_filler ) {
      ...
      const double lep_frWeight = selLepton->isGenMatched(false) ? 1. : evtWeightRecorder.get_jetToLepton_FR_lead(central_or_shift_main);
      ...
      const double tau2_frWeight = selHadTau_sublead->isGenMatched(false) ? 1. : evtWeightRecorder.get_jetToTau_FR_sublead(central_or_shift_main);
      const double evt_frWeight = lep_frWeight * tau1_frWeight * tau2_frWeight;
