...
//--- read python configuration parameters
  ...
  edm::ParameterSet cfg_analyze = cfg.getParameter<edm::ParameterSet>("analyze_jetToTauFakeRateTTemu");
  ...
  if ( absEtaBins.size() < 2 ) throw cms::Exception("analyze_jetToTauFakeRateTTemu") 
    << "Invalid Configuration parameter 'absEtaBins' !!\n";
  vint decayModes = cfg_analyze.getParameter<vint>("decayModes");
  if ( decayModes.size() < 1 ) throw cms::Exception("analyze_jetToTauFakeRateTTemu") 
...
//--- declare particle collections
  ...
  RecoMuonCollectionSelectorFakeable fakeableMuonSelector_default(era);
  RecoMuonCollectionSelectorFakeable_hh_multilepton fakeableMuonSelector_hh_multilepton(era);
  ...
  RecoElectronCollectionSelectorFakeable fakeableElectronSelector_default(era);
  RecoElectronCollectionSelectorFakeable_hh_multilepton fakeableElectronSelector_hh_multilepton(era);
...
//--- open output file containing run:lumi:event numbers of events passing final event selection criteria
  ...
  EvtHistManager_jetToTauFakeRateTTemu selEvtHistManager(makeHistManager_cfg(process_string, 
...
//--- apply HLT filter
    ...
    cutFlowTable.update("lead lepton pT > 25 GeV && sublead lepton pT > 15 GeV", evtWeightRecorder.get(central_or_shift));
...
//--- find jet matching the tau candidate
      ...
      preselHadTau_ptrs.push_back(*preselHadTau);
