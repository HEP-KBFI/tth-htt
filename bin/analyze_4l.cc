...
//--- read python configuration parameters
  ...
  const bool do_tree = (isSignal || process_string == "TTZ" ) && !(central_or_shifts_local.size() > 1);
  ...
  switch(era)
  {
    ...
    default: throw cmsException("analyze_4l", __LINE__) << "Invalid era = " << static_cast<int>(era);
  }
...
//--- declare histograms
  struct selHistManagerType
  {
    ...
    std::map<std::string, std::map<std::string, EvtHistManager_4l*>> evt_htxs_;
    std::map<std::string, std::map<std::string, EvtHistManager_4l*>> evt_in_decayModes_;
    std::map<std::string, std::map<std::string, std::map<std::string, EvtHistManager_4l*>>> evt_htxs_in_decayModes_;
    ...
    for (const GenMatchEntry * genMatchDefinition : genMatchDefinitions)
    {
      ...
      for(const std::string & evt_cat_str: evt_cat_strs)
      {
        ...
        for(const auto & kv: evt_htxs_binning)
        {
          ...
          selHistManager->evt_htxs_[evt_cat_str][kv.first] = new EvtHistManager_4l(makeHistManager_cfg(
	  ...
	  decayMode_and_genMatch += genMatchDefinition->getName();
          ...
          for(const std::string & evt_cat_str: evt_cat_strs)
          {
            ...
            for(const auto & kv: evt_htxs_binning)
            {
              ...
              selHistManager->evt_htxs_in_decayModes_[evt_cat_str][decayMode_evt][kv.first] = new EvtHistManager_4l(makeHistManager_cfg(
...
//--- apply HLT filter
    ...
    //--- compute output of BDTs used to discriminate ttH vs. ttbar trained by Matthias for 1l_2tau category
        ...
        const std::map<std::string, double> mvaInputsValues = {
          ...
          {"lep4_conePt",    comp_lep_conePt(*selLepton_fourth)},
...
//--- fill histograms with events passing final selection
    ...
    for(const std::string & central_or_shift: central_or_shifts_local)
    {
	  ...
	  selHistManager->metFilters_->fillHistograms(metFilters, evtWeight);
        ...
        {
          const EvtHistManager_4l_Input fillVariables {
            selElectrons.size(),
            selMuons.size(),
            selJets.size(),
            selBJets_loose.size(),
          ...
          EvtHistManager_4l* selHistManager_evt = selHistManager->evt_[kv.first];
          ...
          for(const auto & kw: evt_htxs_binning)
          {
            ...
            {
              EvtHistManager_4l_Input fillVariables_htxs = fillVariables;
          ...
          if(isSignal)
          {
            ...
            {
              EvtHistManager_4l* selHistManager_evt_decay = selHistManager -> evt_in_decayModes_[kv.first][decayModeStr];
    ...
    if ( bdt_filler ) {
      ...
      double prob_fake_lepton_fourth = 1.0; //evtWeightRecorder.get_jetToLepton_FR_fourth(central_or_shift_main);
      ...
      bdt_filler -> operator()({ eventInfo.run, eventInfo.lumi, eventInfo.event })
          ...
          ("mindr_lep3_jet",      TMath::Min(10., comp_mindr_jet(*selLepton_third, selJets)))
          ...
          ("mindr_lep4_jet",      TMath::Min(10., comp_mindr_jet(*selLepton_fourth, selJets)))
          ("mT_lep4",             comp_MT_met(selLepton_fourth, met.pt(), met.phi()))
          ...
          ("met_LD",              met_LD)
          ("massLT2",              comp_massL2(selLeptons[0], selLeptons[1], met.pt(), met.phi()))
          ("massLT",              comp_massL4(selLeptons[0], selLeptons[1], selLeptons[2], selLeptons[3], met.pt(), met.phi()))
          ("massL_FO",           massL(fakeableLeptons))
          ("massL4",           massL(selLeptons))
          ("mindr_lep_jet",   std::min(comp_mindr_jet(*selLepton_lead, selJets), std::min(comp_mindr_jet(*selLepton_sublead, selJets), std::min(comp_mindr_jet(*selLepton_third, selJets), std::min(comp_mindr_jet(*selLepton_fourth, selJets), 400.)))))
    ...
    if(snm)
    {
      ...
      const double mT_lep4        = comp_MT_met(selLepton_fourth, met.pt(), met.phi());
