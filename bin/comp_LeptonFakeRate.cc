...
struct fitResultType
{
  ...
  double norm_TT_fakes2_;
  double normErr_TT_fakes2_;
                            ...
                            double avFakeRate_QCD, double avFakeRateErrUp_QCD, double avFakeRateErrDown_QCD, double avFakeRateErr_QCD)
{
  ...
  compFakeRate(nPass_QCD, nPassErr_QCD, nFail_QCD, nFailErr_QCD, avFakeRate_QCD, avFakeRateErrUp_QCD, avFakeRateErrDown_QCD, errorFlag, 1.e-2); // Cap Unc.s by 0.01
...
void readPrefit(TFile* inputFile_stage2, TFile* inputFile_stage1_5, std::map<std::string, fitResultType*>& fitResults, const std::string& variable, bool enable_MC_Closure_sidebands = false)
{
  ...
  for ( std::map<std::string, fitResultType*>::iterator fitResult = fitResults.begin();
    ...
    if(enable_MC_Closure_sidebands){
      ...
      std::cout<< " MC Closure sidebands disabled. Skipping DiLeptSS TT fakes histogram " << std::endl;
    }

    // LeptonPlusJet QCD MC fakes
    ...
    if(enable_MC_Closure_sidebands){ 
      ...
      std::cout<< " MC Closure sidebands disabled. Skipping LeptonPlusJet QCD MC fakes histogram " << std::endl;
    }

    // LeptonPlusJet QCD MC fakes (Conv. corrected)
    ...
    if(enable_MC_Closure_sidebands){  
      ...
      std::cout<< " MC Closure sidebands disabled. Skipping LeptonPlusJet QCD MC fakes (Conv. corrected) histogram " << std::endl;
    }
		   ...
		   bool use_Conv_Corrected_QCD = false, bool is_TTj_minus_TTg = false, bool is_TT_fakes2 = false)
{
  ...
  for ( int idxBinX = 1; idxBinX <= xAxis->GetNbins(); ++idxBinX ) {
	...
	double nFail = 0.;
	double nFailErr = 0.;
	if(is_data_fakes && !is_TT_fakes && !is_QCD_fakes && !is_TTj_minus_TTg && !is_TT_fakes2){ // For data driven fakes
	...
	}else if(!is_data_fakes && is_TT_fakes && !is_QCD_fakes && !is_TTj_minus_TTg && !is_TT_fakes2){ // For TTBar MC driven fakes
	...
	}else if(!is_data_fakes && !is_TT_fakes && !is_QCD_fakes && is_TTj_minus_TTg && !is_TT_fakes2){ // For TTj - TTg
	    std::cout<< "(TTj - TTg) or Conversion corrected TTbar MC driven fakes " << std::endl;
	...
	std::cout << " nPass2 " << nPass << " +/- " << nPassErr << " nFail2 " <<  nFail  << " +/- " << nFailErr << std::endl;
        compFakeRate(nPass, nPassErr, nFail, nFailErr, avFakeRate, avFakeRateErrUp, avFakeRateErrDown, errorFlag, 1.e-2); // Cap Unc.s by 0.01
...
void makeControlPlot(TGraphAsymmErrors* graph_data, 
		     TGraphAsymmErrors* graph_mc_QCD_fakes,  
		     TGraphAsymmErrors* graph_mc_QCD_fakes_Conv_Corr, 
		     TGraphAsymmErrors* graph_mc_TT_fakes, 
		     TGraphAsymmErrors* graph_mc_TTj_minus_TTg_fakes,
		     TGraphAsymmErrors* graph_mc_TT_fakes2, 
		     ...
		     bool useLogScale, const std::string& outputFileName)
{
  ...
  if ( graph_mc_TT_fakes2 ) {
    graph_mc_TT_fakes2->SetMarkerStyle(30);
    graph_mc_TT_fakes2->SetMarkerSize(2);
    graph_mc_TT_fakes2->SetMarkerColor(1);
    graph_mc_TT_fakes2->SetLineStyle(1);
    graph_mc_TT_fakes2->SetLineWidth(1);
    graph_mc_TT_fakes2->SetLineColor(1);
    graph_mc_TT_fakes2->Draw("psame");
  }
  ...
  TLegend* legend = 0;
    ...
    if ( graph_mc_TT_fakes2 ) legend->AddEntry(graph_mc_TT_fakes2,   "Fakes (t#bar{t}:DiLeptSS)", "p");
    legend->Draw();
...
int main(int argc, char* argv[])
{
  ...
  // boolean handle to enable/disable MC closure sideband inclusion                                                                                                                                       
  const bool enable_MC_Closure_sidebands = cfg_comp.getParameter<bool>("enable_MC_Closure_sidebands");
  ...
  for ( int idxBinY = 1; idxBinY <= yAxis_e->GetNbins(); ++idxBinY ) {
    ...
    if(enable_MC_Closure_sidebands){
      //---- PLOTTING CONV. CORREC. QCD/TTBAR AND OLD (DATA-DRIVEN) TTBAR GRAPHS
      ...
      // ---- OMITTING CONV. CORREC. QCD/TTBAR AND OLD (DATA-DRIVEN) TTBAR GRAPHS
      ...
      // ---- OMITTING ALL MC DRIVEN GRAPHS
      makeControlPlot(graph_data, 0, 0, 0, 0, 0, ptBins_e_array, minAbsEta, maxAbsEta, true, outputFileName_plot);
      makeControlPlot(graph_data, 0, 0, 0, 0, 0, ptBins_e_array, minAbsEta, maxAbsEta, false, outputFileName_plot);
    }
  ...
  for ( int idxBinY = 1; idxBinY <= yAxis_mu->GetNbins(); ++idxBinY ) {
    ...
    if(enable_MC_Closure_sidebands){
      //---- PLOTTING OLD (DATA-DRIVEN) TTBAR GRAPH
      ...
      // ---- OMITTING OLD (DATA-DRIVEN) TTBAR GRAPH
      ...
      // ---- OMITTING ALL MC DRIVEN GRAPHS
      makeControlPlot(graph_data, 0, 0, 0, 0, 0, ptBins_mu_array, minAbsEta, maxAbsEta, true, outputFileName_plot);
      makeControlPlot(graph_data, 0, 0, 0, 0, 0, ptBins_mu_array, minAbsEta, maxAbsEta, false, outputFileName_plot);
    }
