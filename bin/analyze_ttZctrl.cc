...
//--- declare histograms
  struct selHistManagerType
  {
    ...
    std::map<std::string, EvtHistManager_ttZctrl*> evt_;
    std::map<std::string, std::map<std::string, EvtHistManager_ttZctrl*>> evt_in_decayModes_;
  ...
  for(const std::string & central_or_shift: central_or_shifts_local)
  {
    for(const leptonGenMatchEntry & leptonGenMatch_definition: leptonGenMatch_definitions)
    {
      ...
      for(const std::string & evt_cat_str: evt_cat_strs)
      {
        ...
        selHistManager->evt_[evt_cat_str] = new EvtHistManager_ttZctrl(makeHistManager_cfg(
      ...
      if ( isSignal ) {
        for ( const std::string & decayMode_evt: decayModes_evt ) {
          ...
          for(const std::string & evt_cat_str: evt_cat_strs)
          {
            ...
            selHistManager -> evt_in_decayModes_[evt_cat_str][decayMode_evt] = new EvtHistManager_ttZctrl(makeHistManager_cfg(
...
//--- apply HLT filter
    ...
    if(apply_met_filters && !metFilterSelector(metFilters))
    {
      if(run_lumi_eventSelector)
      {
...
//    in 2lss category of ttH multilepton analysis
    ...
    double mvaOutput_2lss_ttbar = 1.0;
