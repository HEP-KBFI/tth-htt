...
class analyzeConfig:
  ...
  def __init__(self, output_dir, exec_name, hadTau_selections, charge_selections, central_or_shifts,
    ...
    self.datacard_outputfile = os.path.join(self.output_dir, DKEY_DCRD, "prepareDatacards_1l_2tau_%s.root")
    self.prep_dcard_cfg_fullpath = os.path.join(self.output_dir, DKEY_CFGS, "prepareDatacards_1l_2tau_%s_cfg.py")
...
def create_setup(cfg):
  ...
  for histogramToFit in cfg.histograms_to_fit:
    prep_dcard_cfg_contents = create_prep_dcard_cfg(cfg.histogram_file_hadd_stage2, cfg.datacard_outputfile % histogramToFit,
      "1l_2tau_OS_Tight", cfg.output_category, histogramToFit)
