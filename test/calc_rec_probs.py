...
def calc_rec_probs(infile, processes):
  ...
  for p in processes:
    ...
    for cat in categories:
        histo_OS = f.Get("OS/%s/%s"  % (cat, p))
        histo_SS = f.Get("SS/%s/%s" % (cat, p))
...
def calc_probs_21(infile):
  ...
  for cat in cats:
    histo_OS = f.Get("gen/OS/%s/DY" % cat)
    histo_SS = f.Get("gen/SS/%s/DY" % cat)
    os_count = histo_OS.Integral()
    ss_count = histo_SS.Integral()
...
def print_probs_21(infile):
  ...
  for cat in cats:
    ...
    os_count = histo_OS.Integral()
    ss_count = histo_SS.Integral()
    if os_count > 0:ratio = ss_count / (ss_count + os_count)
    else: ratio = 1.
    print "%d, %f, %f, %f" % (i, ratio, 0., 0.)
    i+=1
        

if __name__ == "__main__":
  procs = ["DY"]
  infile = "/home/andres/tth/histosCF_pseudodata_DYgenplus/histograms/charge_flip_Tight/allHistogramsCF.root"
