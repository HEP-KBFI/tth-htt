...
void RecoMuonReader::setBranchNames()
{
  if (numInstances_[branchName_obj_] == 0) {
    ...
    if      (era_ == kEra_2015) branchName_mediumIdPOG_ = Form("%s_%s",
                                                               branchName_obj_.data(),
                                                               "mediumMuonId");
    else if (era_ == kEra_2016) branchName_mediumIdPOG_ = Form("%s_%s",
                                                               branchName_obj_.data(),
                                                               "mediumIdPOG_ICHEP2016");
...
#endif // ifdef DPT_DIV_PT
    branchName_segmentCompatibility_ = Form("%s_%s",
                                            branchName_obj_.data(),
    ...
    if (branchName_num_ != instances_[branchName_obj_]->branchName_num_) {
      throw cms::Exception("RecoMuonReader")
            ...
            instances_[branchName_obj_]->branchName_num_ <<
            " with 'branchName_obj' = " <<
...
void RecoMuonReader::setBranchAddresses(TTree *tree)
{
  if (instances_[branchName_obj_] == this) {
    ...
    tree->SetBranchAddress(
      branchName_segmentCompatibility_.data(), segmentCompatibility_);
  }
}
...
std::vector<RecoMuon>RecoMuonReader::read() const
{
  ...
  if (nLeptons > leptonReader_->max_nLeptons_) {
    throw cms::Exception("RecoMuonReader")
          ...
          ", exceeds max_nLeptons = " << leptonReader_->max_nLeptons_ << " !!\n";
  }
