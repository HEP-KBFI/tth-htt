  ...
  , hadTau3_decayMode_(-1)
{
  ...
  if ( central_or_shift != "central" ) {
    ...
    // CV: no systematic uncertainties on data/MC corrections for trigger efficiency in 0l_3tau category defined yet
  }
...
double Data_to_MC_CorrectionInterface_0l_3tau_trigger::getSF_triggerEff() const
{
  if ( isDEBUG_ ) {
  ...
  bool hadTau3_isGenTau = (hadTau3_genPdgId_ == 11 || hadTau3_genPdgId_ == 13 || hadTau3_genPdgId_ == 15);
  if ( hadTau1_isGenTau ) {
    eff_2tau_tau1_data = get_from_lut(effTrigger_2tau_perLeg_data_gentau_, hadTau1_pt_, hadTau1_eta_, hadTau1_decayMode_, isDEBUG_);
    eff_2tau_tau1_mc = get_from_lut(effTrigger_2tau_perLeg_mc_gentau_, hadTau1_pt_, hadTau1_eta_, hadTau1_decayMode_, isDEBUG_);
  } else {
    eff_2tau_tau1_data = get_from_lut(effTrigger_2tau_perLeg_data_faketau_, hadTau1_pt_, hadTau1_eta_, hadTau1_decayMode_, isDEBUG_);
    eff_2tau_tau1_mc = get_from_lut(effTrigger_2tau_perLeg_mc_faketau_, hadTau1_pt_, hadTau1_eta_, hadTau1_decayMode_, isDEBUG_);
  }
  if ( hadTau2_isGenTau ) {
    eff_2tau_tau2_data = get_from_lut(effTrigger_2tau_perLeg_data_gentau_, hadTau2_pt_, hadTau2_eta_, hadTau2_decayMode_, isDEBUG_);
    eff_2tau_tau2_mc = get_from_lut(effTrigger_2tau_perLeg_mc_gentau_, hadTau2_pt_, hadTau2_eta_, hadTau2_decayMode_, isDEBUG_);
  } else {
    eff_2tau_tau2_data = get_from_lut(effTrigger_2tau_perLeg_data_faketau_, hadTau2_pt_, hadTau2_eta_, hadTau2_decayMode_, isDEBUG_);
    eff_2tau_tau2_mc = get_from_lut(effTrigger_2tau_perLeg_mc_faketau_, hadTau2_pt_, hadTau2_eta_, hadTau2_decayMode_, isDEBUG_);
  }
  if ( std::fabs(hadTau3_eta_) < 2.1 ) { 
    if ( hadTau3_isGenTau ) {
      eff_2tau_tau3_data = get_from_lut(effTrigger_2tau_perLeg_data_gentau_, hadTau3_pt_, hadTau3_eta_, hadTau3_decayMode_, isDEBUG_);
      eff_2tau_tau3_mc = get_from_lut(effTrigger_2tau_perLeg_mc_gentau_, hadTau3_pt_, hadTau3_eta_, hadTau3_decayMode_, isDEBUG_);
    } else {
      eff_2tau_tau3_data = get_from_lut(effTrigger_2tau_perLeg_data_faketau_, hadTau3_pt_, hadTau3_eta_, hadTau3_decayMode_, isDEBUG_);
      eff_2tau_tau3_mc = get_from_lut(effTrigger_2tau_perLeg_mc_faketau_, hadTau3_pt_, hadTau3_eta_, hadTau3_decayMode_, isDEBUG_);
    }
  }
  ...
  if ( eff_2tau_data < 0. ) eff_2tau_data = 0.;
  if ( eff_2tau_data > 1. ) eff_2tau_data = 1.;
  double eff_2tau_mc = 
  ...
  if ( eff_2tau_mc < 0. ) eff_2tau_mc = 0.;
  if ( eff_2tau_mc > 1. ) eff_2tau_mc = 1.;
  if ( isDEBUG_ ) {
    ...
    std::cout << "hadTau (third): pT = " << hadTau3_pt_ << ", eta = " << hadTau3_eta_ << std::endl;    
  }
  double sf = 1.;
