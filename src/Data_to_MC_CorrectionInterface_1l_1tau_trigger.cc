...
namespace
{
  ...
  std::string getHadTauSelectionLabel(const std::string& hadTauSelection)
  {
    ...
    else throw cms::Exception("Data_to_MC_CorrectionInterface_1l_1tau_trigger")
  ...
  , hadTau_decayMode_(-1)
{
  ...
  if ( central_or_shift != "central" ) {
    ...
    // CV: no systematic uncertainties on data/MC corrections for trigger efficiency in 1l_1tau category defined yet
  }
...
void Data_to_MC_CorrectionInterface_1l_1tau_trigger::setTriggerBits(bool isTriggered_1e, bool isTriggered_1e1tau, bool isTriggered_1m, bool isTriggered_1m1tau)
{
...
void Data_to_MC_CorrectionInterface_1l_1tau_trigger::setLeptons(int lepton_type, double lepton_pt, double lepton_eta)
{
...
void Data_to_MC_CorrectionInterface_1l_1tau_trigger::setHadTaus(int hadTau_genPdgId, double hadTau_pt, double hadTau_eta, int hadTau_decayMode)
{
...
double Data_to_MC_CorrectionInterface_1l_1tau_trigger::getSF_triggerEff() const
{
  if ( isDEBUG_ ) {
    std::cout << "<Data_to_MC_CorrectionInterface_1l_1tau_trigger::getSF_triggerEff>:" << std::endl;
  }
  double sf = 1.;
  if ( era_ == kEra_2016 ) {
    ...
    bool hadTau_isGenTau = (hadTau_genPdgId_ == 11 || hadTau_genPdgId_ == 13 || hadTau_genPdgId_ == 15);
    if ( lepton_type_ == kElectron ) {
      if ( isDEBUG_ ) {
	std::cout << "electron: pT = " << lepton_pt_ << ", eta = " << lepton_eta_ << std::endl;
      }
	...
	eff_1l1tau_tauLeg_mc = get_from_lut(effTrigger_1e1tau_tauLeg_mc_faketau_, hadTau_pt_, hadTau_eta_, hadTau_decayMode_, isDEBUG_);
      ...
      if ( isDEBUG_ ) {
	std::cout << "muon: pT = " << lepton_pt_ << ", eta = " << lepton_eta_ << std::endl;
      }
    ...
    if ( isDEBUG_ ) {
      std::cout << "hadTau: pT = " << hadTau_pt_ << ", eta = " << hadTau_eta_ << std::endl;
      std::cout << "eff (L): data = " << eff_1l_data << ", MC = " << eff_1l_mc << std::endl;
      std::cout << "eff (X_lep): data = " << eff_1l1tau_lepLeg_data << ", MC = " << eff_1l1tau_lepLeg_mc << std::endl;
      std::cout << "eff (X_tau): data = " << eff_1l1tau_tauLeg_data << ", MC = " << eff_1l1tau_tauLeg_mc << std::endl;  
    }
    //-------------------------------------------------------------------------------------------------------------------
    ...
    if ( isTriggered_1l && isTriggered_1l1tau ) { // case 4: both single lepton trigger and lepton+tau cross trigger fire
      double eff_data = TMath::Min(eff_1l_data, eff_1l1tau_lepLeg_data)*eff_1l1tau_tauLeg_data;
      double eff_mc = TMath::Min(eff_1l_mc, eff_1l1tau_lepLeg_mc)*eff_1l1tau_tauLeg_mc;      
      sf = compSF(eff_data, eff_mc);
      if ( isDEBUG_ ) {
	std::cout << "case 4: both single lepton trigger and lepton+tau cross trigger fire" << std::endl;
	std::cout << " eff: data = " << eff_data << ", MC = " << eff_mc << " --> SF = " << sf << std::endl;
      }
    } else if ( isTriggered_1l1tau ) {            // case 3: lepton+tau cross trigger fires, single lepton trigger doesn't fire
      double eff_data = TMath::Max(1.e-2, (eff_1l1tau_lepLeg_data - eff_1l_data)*eff_1l1tau_tauLeg_data);
      double eff_mc = TMath::Max(1.e-2, (eff_1l1tau_lepLeg_mc - eff_1l_mc)*eff_1l1tau_tauLeg_mc);
      sf = compSF(eff_data, eff_mc);
      if ( isDEBUG_ ) {
	std::cout << "case 3: lepton+tau cross trigger fires, single lepton trigger doesn't fire" << std::endl;
	std::cout << " eff: data = " << eff_data << ", MC = " << eff_mc << " --> SF = " << sf << std::endl;
      }
    } else if ( isTriggered_1l ) {                // case 2: single lepton trigger fires, lepton+tau cross trigger doesn't fire
      double eff_data = TMath::Max(1.e-2, eff_1l_data - eff_1l1tau_tauLeg_data*TMath::Min(eff_1l_data, eff_1l1tau_lepLeg_data));
      double eff_mc = TMath::Max(1.e-2, eff_1l_mc - eff_1l1tau_tauLeg_mc*TMath::Min(eff_1l_mc, eff_1l1tau_lepLeg_mc));
      sf = compSF(eff_data, eff_mc);
      if ( isDEBUG_ ) {
	std::cout << "case 2: single lepton trigger fires, lepton+tau cross trigger doesn't fire" << std::endl;
	std::cout << " eff: data = " << eff_data << ", MC = " << eff_mc << " --> SF = " << sf << std::endl;
      }
      ...
      sf = 0.;
      if ( isDEBUG_ ) {
	std::cout << "case 1: neither single lepton trigger nor lepton+tau cross trigger fires" << std::endl;
