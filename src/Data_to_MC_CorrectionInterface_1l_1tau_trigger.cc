  ...
  if(era_ == kEra_2017)
  {
    ...
    for(int idxEtaBin = 0; idxEtaBin < numEtaBins_1e_1tau_lepLeg; ++idxEtaBin)
    {
      ...
      effTrigger_1e1tau_lepLeg_data_.push_back(new lutWrapperTGraph(
        inputFiles_, "tthAnalysis/HiggsToTauTau/data/triggerSF/2016/Electron_Ele24_eff.root",
        Form("ZMassEta%s_Data", etaBinLabel.data()),
        lut::kXptYabsEta, -1., -1., etaMin, etaMax
      ));
...
void
Data_to_MC_CorrectionInterface_1l_1tau_trigger::setHadTaus(int hadTau_genPdgId,
                                                           double hadTau_pt,
                                                           double hadTau_eta,
...
Data_to_MC_CorrectionInterface_1l_1tau_trigger::getSF_triggerEff() const
{
  if(isDEBUG_)
  {
    std::cout << "<Data_to_MC_CorrectionInterface_1l_1tau_trigger::getSF_triggerEff>:\n";
  }
  ...
  if(era_ == kEra_2017)
  {
    ...
    if(lepton_type_ == kElectron)
    {
      ...
      if(hadTau_isGenTau)
      {
        eff_1l1tau_tauLeg_data = aux::get_from_lut(effTrigger_1e1tau_tauLeg_data_gentau_, hadTau_pt_, hadTau_eta_, hadTau_decayMode_, isDEBUG_);
        eff_1l1tau_tauLeg_mc   = aux::get_from_lut(effTrigger_1e1tau_tauLeg_mc_gentau_,   hadTau_pt_, hadTau_eta_, hadTau_decayMode_, isDEBUG_);
      }
      else
      {
        eff_1l1tau_tauLeg_data = aux::get_from_lut(effTrigger_1e1tau_tauLeg_data_faketau_, hadTau_pt_, hadTau_eta_, hadTau_decayMode_, isDEBUG_);
        eff_1l1tau_tauLeg_mc   = aux::get_from_lut(effTrigger_1e1tau_tauLeg_mc_faketau_,   hadTau_pt_, hadTau_eta_, hadTau_decayMode_, isDEBUG_);
      }
    ...
    if(isDEBUG_)
    {
      std::cout << "hadTau: pT = " << hadTau_pt_ << ", eta = " << hadTau_eta_                            << "\n"
