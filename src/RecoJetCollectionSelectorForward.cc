...
void
RecoJetSelectorForward::set_min_pt(double min_pt)
{
  min_pt_ = min_pt;
}

void
RecoJetSelectorForward::set_max_absEta(double max_absEta)
{
  max_absEta_ = max_absEta;
}

void
RecoJetSelectorForward::set_min_jetId(int min_jetId)
{
  min_jetId_ = min_jetId;
}

double
RecoJetSelectorForward::get_min_pt() const
{
  return min_pt_;
}

double
RecoJetSelectorForward::get_max_absEta() const
{
  return max_absEta_;
}
...
int
RecoJetSelectorForward::get_min_jetId() const
{
  return min_jetId_;
}
...
RecoJetSelectorForward::operator()(const RecoJet & jet) const
{
  ...
  if(min_absEta_ > 0. && jet.absEta() < min_absEta_)
  {
    if(debug_)
    {
      std::cout << "FAILS abs(eta) >= " << min_absEta_ << " cut\n";
    }
    return false;
  }
