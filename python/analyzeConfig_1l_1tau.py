  ...
  def create(self):
    ...
    for process_name in self.nonfake_backgrounds:
      for hadTau_selection in self.hadTau_selections:
        ...
        for charge_selection in self.charge_selections:
          key = getKey(process_name, hadTau_selection, charge_selection)
          self.histogramFile_addBackgrounds[key] = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_%s_%s_%s.root" % \
            (self.channel, process_name, hadTau_selection, charge_selection))        
          ...
          histogramDir = "1l_2tau_%s_%s" % (charge_selection, hadTau_selection_part1)
    ...
    for charge_selection in self.charge_selections:
      ...
      category_sideband = "1l_1tau_%s_Fakeable" % charge_selection
