...
def getHistogramDir(hadTau_selection, hadTau_frWeight, charge_selection):
  ...
  histogramDir = "1l_1tau_%s_%s" % (charge_selection, hadTau_selection_part1)
...
class analyzeConfig_1l_1tau(analyzeConfig):
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, era, triggers, charge_selection,
    ...
    histogramDir = getHistogramDir(hadTau_selection, hadTau_frWeight, charge_selection)
    ...
    lines.append("process.analyze_1l_1tau.apply_hadTauGenMatching = cms.bool(%s)" % apply_hadTauGenMatching)
    if hadTau_frWeight == "enabled":
      lines.append("process.analyze_1l_1tau.applyJetToTauFakeRateWeight = cms.bool(True)")
      if era == "2015":
        lines.append("process.analyze_1l_1tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2015.root')")
        ...
        lines.append("process.analyze_1l_1tau.jetToTauFakeRateWeight.lead.graphName = cms.string('%s'" % graphName)
        lines.append("process.analyze_1l_1tau.jetToTauFakeRateWeight.lead.fitFunctionName = cms.string('%s'" % fitFunctionName)
      elif era == "2016":
        lines.append("process.analyze_1l_1tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2016.root')")
      else:
        raise ValueError("Invalid parameter 'era' = %s !!" % era)
    elif hadTau_frWeight == "disabled":
      lines.append("process.analyze_1l_1tau.applyJetToTauFakeRateWeight = cms.bool(False)")
    else:
      raise ValueError("Invalid parameter 'hadTau_frWeight' = %s !!" % hadTau_frWeight)
    if hadTau_selection.find("mcClosure") != -1:
      lines.append("process.analyze_1l_1tau.jetToTauFakeRateWeight.applyFitFunction_lead = cms.bool(False)")
  ...
  def createCfg_makePlots_mcClosure(self):
    ...
    lines.append("  ),")
    lines.append("  cms.PSet(")
    lines.append("    histogramName = cms.string('sel/evt/$PROCESS/mvaOutput_1l_1tau_ttbar'),")
  ...
  def create(self):
    ...
    for process_name in process_names:
      for hadTau_selection in self.hadTau_selections:
        for hadTau_frWeight in self.hadTau_frWeights:
          ...
          hadTau_selection_and_frWeight = get_hadTau_selection_and_frWeight(hadTau_selection, hadTau_frWeight)
          for charge_selection in self.charge_selections:
            key = getKey(process_name, hadTau_selection, hadTau_frWeight, charge_selection)
            self.histogramFile_addBackgrounds[key] = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_%s_%s_%s.root" % \
              (self.channel, process_name, hadTau_selection_and_frWeight, charge_selection))        
    ...
    for hadTau_selection in self.hadTau_selections:
      for hadTau_frWeight in self.hadTau_frWeights:
        ...
        hadTau_selection_and_frWeight = get_hadTau_selection_and_frWeight(hadTau_selection, hadTau_frWeight)
        for charge_selection in self.charge_selections:
          key = getKey(hadTau_selection, hadTau_frWeight, charge_selection)
          self.histogramFile_addBackgrounds[key] = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_fakes_mc_%s_%s.root" % \
            (self.channel, hadTau_selection_and_frWeight, charge_selection))
    ...
    for charge_selection in self.charge_selections:
      ...
      self.cfgFile_addFakes_modified[key] = os.path.join(self.outputDir, DKEY_CFGS, "addBackgroundJetToTauFakes_%s_%s_cfg.py" % \
        (self.channel, charge_selection))
      category_signal = "1l_1tau_%s_Tight" % charge_selection
      category_sideband = "1l_1tau_%s_Fakeable_wFakeRateWeights" % charge_selection
