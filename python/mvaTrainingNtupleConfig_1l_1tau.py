...
import tthMVATrainingNtupleSamples_1l_1tau
...
class mvaTrainingNtupleConfig_1l_1tau(analyzeConfig):
  """Configuration metadata needed to run analysis in a single go.
  
  Sets up a folder structure by defining full path names; no directory creation is delegated here.
  
  Args specific to mvaTrainingNtupleConfig_1l_1tau:
  ...
  def __init__(self, outputDir, executable_analyze, charge_selections, hadTau_selections, central_or_shifts,
    ...
    self.samples = tthMVATrainingNtupleSamples_1l_1tau.samples
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "produceTTHMVATrainingNtuple_1l_1tau_cfg.py")
    self.histogramDir_prep_dcard = "ntuple_1l_1tau_OS_Tight"
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, triggers, charge_selection, hadTau_selection, hadTau_genMatch, hadTau_frWeight, 
    ...
    """Create python configuration file for the produceTTHMVATrainingNtuple_1l_1tau executable (analysis code)
    ...
    Args:
      ...
      central_or_shift: either 'central' or one of the systematic uncertainties defined in $CMSSW_BASE/src/tthAnalysis/HiggsToTauTau/bin/produceTTHMVATrainingNtuple_1l_1tau.cc
    """  
    lines = []
    ...
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.process = cms.string('%s')" % sample_category)
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.use_triggers_1e = cms.bool(%s)" % ("1e" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.use_triggers_1mu = cms.bool(%s)" % ("1mu" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.chargeSelection = cms.string('%s')" % charge_selection)
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.hadTauSelection = cms.string('%s')" % hadTau_selection)
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.hadTauGenMatch = cms.string('%s')" % hadTau_genMatch)
    if hadTau_frWeight == "enabled":
      lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.applyJetToTauFakeRateWeight = cms.bool(True)")
    elif hadTau_frWeight != "disabled":
      raise ValueError("Invalid parameter 'hadTau_frWeight' = %s !!" % hadTau_frWeight) 
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.isMC = cms.bool(%s)" % is_mc)
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.central_or_shift = cms.string('%s')" % central_or_shift)
    lines.append("process.produceTTHMVATrainingNtuple_1l_1tau.lumiScale = cms.double(%f)" % lumi_scale)
