...
class analyzeConfig_charge_flip_mu(analyzeConfig):
  ...
  def __init__(self, outputDir, executable_analyze, samples, lepton_selections, central_or_shifts,
               ...
               histograms_to_fit = [], select_rle_output = False, executable_prep_dcard="prepareDatacardMu"):
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "analyze_charge_flip_mu_cfg.py")
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, era, triggers, lepton_selection, 
    ...
    Args:
      ...
      central_or_shift: either 'central' or one of the systematic uncertainties defined in $CMSSW_BASE/src/tthAnalysis/HiggsToTauTau/bin/analyze_charge_flip_mu.cc
    """  
    lines = []
  ...
  def createCfg_prep_dcard(self, histogramToFit, histogramDir = None, label = None):
    ...
    """
    datacardFile = os.path.join(self.outputDir, DKEY_DCRD, "prepareDatacards_mu_%s_%s.root" % (self.channel, histogramToFit))
    category_output = self.channel
    cfgFile_modified = os.path.join(self.outputDir, DKEY_CFGS, "prepareDatacards_mu_%s_%s_cfg.py" % (self.channel, histogramToFit))
    key = histogramToFit
    if not histogramDir:
