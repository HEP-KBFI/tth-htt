...
class analyzeConfig_3l_1tau(analyzeConfig):
  ...
  def __init__(self, outputDir, executable_analyze, lepton_selections, hadTau_selection, charge_selections, central_or_shifts,
    ...
    self.histogramDir_prep_dcard = "3l_1tau_OS_Tight"
    self.select_rle_output = select_rle_output
    self.select_root_output = select_root_output

  def createCfg_analyze(self, inputFiles, outputFile, sample_category, triggers, lepton_selection, hadTau_selection, charge_selection,
        ...
        for charge_selection in self.charge_selections:
          for central_or_shift in self.central_or_shifts:
            ...
            for jobId in range(len(self.inputFileIds[sample_name])):
              key_file = getKey(sample_name, lepton_selection, charge_selection, central_or_shift, jobId)
            ...
            if len(inputFiles_jobIds) > 0:
              haddFile_jobIds = self.histogramFile_hadd_stage1.replace(".root", "_%s_%s_%s_%s.root" % \
                (process_name, lepton_selection, charge_selection, central_or_shift))
        ...
        for charge_selection in self.charge_selections:
          ...
          for central_or_shift in self.central_or_shifts:
            ...
            if self.select_root_output:
              rootOutputSingleFile = os.path.join(self.dirs[key_dir][DKEY_ROOT], "out_%s_%s_%s_%s_%s.root" % \
                (self.channel, process_name, lepton_selection, charge_selection, central_or_shift))
              self.rootOutputAux[rootOutputSingleFile] = os.path.join(self.dirs[key_dir][DKEY_ROOT], "out_%s_%s_%s_%s_%s_*.root" % \
                (self.channel, process_name, lepton_selection, charge_selection, central_or_shift))
            ...
            for jobId in range(len(self.inputFileIds[sample_name])):
              ...
              self.cfgFiles_analyze_modified[key_file] = os.path.join(self.dirs[key_dir][DKEY_CFGS], "analyze_%s_%s_%s_%s_%s_%i_cfg.py" % \
                (self.channel, process_name, lepton_selection, charge_selection, central_or_shift, jobId))
              self.histogramFiles[key_file] = os.path.join(self.dirs[key_dir][DKEY_HIST], "%s_%s_%s_%s_%i.root" % \
                (process_name, lepton_selection, charge_selection, central_or_shift, jobId))
              ...
              self.rleOutputFiles[key_file] = os.path.join(self.dirs[key_dir][DKEY_RLES], "rle_%s_%s_%s_%s_%s_%i.txt" % \
                (self.channel, process_name, lepton_selection, charge_selection, central_or_shift, jobId)) if self.select_rle_output else ""
              self.rootOutputFiles[key_file] = os.path.join(self.dirs[key_dir][DKEY_ROOT], "out_%s_%s_%s_%s_%s_%i.root" % \
                (self.channel, process_name, lepton_selection, charge_selection, central_or_shift, jobId)) if self.select_root_output else ""

              self.createCfg_analyze(inputFiles, self.histogramFiles[key_file], sample_category, triggers,
                lepton_selection, self.hadTau_selection, charge_selection, 
