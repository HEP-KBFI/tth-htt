...
class analyzeConfig_0l_3tau(analyzeConfig):
  ...
  def createCfg_analyze(self, jobOptions):
    ...
    for trigger in [ '2tau' ]:
      lines.append("process.analyze_0l_3tau.triggers_%s = cms.vstring(%s)" % \
        (trigger, self.whitelist_triggers(getattr(self, 'triggers_%s' % trigger), jobOptions['process_name_specific'])))
      lines.append("process.analyze_0l_3tau.use_triggers_%s = cms.bool(%s)" % (trigger, trigger in jobOptions['triggers']))
    ...
    if jobOptions['hadTau_selection'].find("Fakeable") != -1 and jobOptions['applyFakeRateWeights'] in [ "3tau" ]:
      ...
      if self.era == "2017":
        ...
        lines.append("process.analyze_0l_3tau.hadTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2016_vvLoosePresel.root')")
    ...
    lines.append("process.analyze_0l_3tau.isDEBUG = cms.bool(%s)" % self.isDebug)
    lines.append("process.analyze_0l_3tau.apply_hlt_filter = cms.bool(%s)" % self.hlt_filter)
