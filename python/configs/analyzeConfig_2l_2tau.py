...
class analyzeConfig_2l_2tau(analyzeConfig):
  ...
  def __init__(self, configDir, outputDir, executable_analyze, cfgFile_analyze, samples,
               ...
               executable_add_syst_dcard = "addSystDatacards", verbose = False, dry_run = False, isDebug = False):
  ...
  def createCfg_analyze(self, jobOptions):
    ...
    for trigger in [ '1e', '1mu', '2e', '2mu', '1e1mu' ]:
      lines.append("process.analyze_2l_2tau.triggers_%s = cms.vstring(%s)" % (trigger, getattr(self, 'triggers_%s' % trigger)))
