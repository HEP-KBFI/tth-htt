...
class analyzeConfig_ttWctrl(analyzeConfig):
  ...
  def createCfg_analyze(self, jobOptions):
    ...
    if self.do_sync:
      lines.append("process.analyze_ttWctrl.syncNtuple.tree   = cms.string('%s')" % jobOptions['syncTree'])
      lines.append("process.analyze_ttWctrl.syncNtuple.output = cms.string('%s')" % os.path.basename(jobOptions['syncOutput']))
