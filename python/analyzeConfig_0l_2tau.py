...
class analyzeConfig_0l_2tau(analyzeConfig):
  ...
  """
  def __init__(self, outputDir, executable_analyze, samples, hadTau_selections, hadTau_charge_selections, central_or_shifts,
    ...
    analyzeConfig.__init__(self, outputDir, executable_analyze, "0l_2tau", central_or_shifts,
      max_files_per_job, era, use_lumi, lumi, debug, running_method, num_parallel_jobs, 
      histograms_to_fit)
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "analyze_0l_2tau_cfg.py")
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, era, triggers,
                        hadTau_selection, hadTau_genMatch, apply_hadTauGenMatching, hadTau_frWeight, hadTau_charge_selection,
    ...
    lines.append("process.analyze_0l_2tau.apply_hadTauGenMatching = cms.bool(%s)" % apply_hadTauGenMatching)
    if hadTau_frWeight == "enabled":
      lines.append("process.analyze_0l_2tau.applyJetToTauFakeRateWeight = cms.bool(True)")
      if era == "2015":
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2015.root')")
        ...
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.lead.graphName = cms.string('%s')" % graphName)
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.lead.fitFunctionName = cms.string('%s')" % fitFunctionName)
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.sublead.graphName = cms.string('%s')" % graphName)
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.sublead.fitFunctionName = cms.string('%s')" % fitFunctionName)
      elif era == "2016":
        lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2016.root')")
      else:
        raise ValueError("Invalid parameter 'era' = %s !!" % era)
    elif hadTau_frWeight == "disabled":
      lines.append("process.analyze_0l_2tau.applyJetToTauFakeRateWeight = cms.bool(False)")
    else:
      raise ValueError("Invalid parameter 'hadTau_frWeight' = %s !!" % hadTau_frWeight)
    if hadTau_selection.find("mcClosure") != -1:
      lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.applyFitFunction_lead = cms.bool(False)")
      lines.append("process.analyze_0l_2tau.jetToTauFakeRateWeight.applyFitFunction_sublead = cms.bool(False)")
    lines.append("process.analyze_0l_2tau.isMC = cms.bool(%s)" % is_mc)
    lines.append("process.analyze_0l_2tau.central_or_shift = cms.string('%s')" % central_or_shift)
    lines.append("process.analyze_0l_2tau.lumiScale = cms.double(%f)" % lumi_scale)
    lines.append("process.analyze_0l_2tau.apply_trigger_bits = cms.bool(%s)" % apply_trigger_bits)
  ...
  def createCfg_makePlots_mcClosure(self):
    ...
    lines.append("  ),")
    lines.append("  cms.PSet(")
    lines.append("    histogramName = cms.string('sel/evt/$PROCESS/mvaOutput_0l_2tau_ttbar'),")
    ...
    lines.append("  ),")
    lines.append("  cms.PSet(")
    lines.append("    histogramName = cms.string('sel/evt/$PROCESS/mTauTau'),")
    lines.append("    xAxisTitle = cms.string('m_{#tau#tau} [GeV]'),")
    lines.append("    yAxisTitle = cms.string('dN/dm_{#tau#tau} [1/GeV]')")
    lines.append("  )")
    lines.append(")")
