...
def getHistogramDir(lepton_selection, hadTau_selection, hadTau_frWeight):
  ...
  histogramDir = "2los_1tau_lep%s_tau%s" % (lepton_selection, hadTau_selection_part1)
...
class analyzeConfig_2los_1tau(analyzeConfig):
  ...
  def __init__(self, outputDir, executable_analyze, samples, hadTau_selections, central_or_shifts,
    ...
    self.histogramDir_prep_dcard = "2los_1tau_lepTight_tauTight"
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, era, triggers,
                        lepton_selection, hadTau_selection, hadTau_genMatch, apply_hadTauGenMatching, hadTau_frWeight, 
    ...
    lines.append("process.analyze_2los_1tau.process = cms.string('%s')" % sample_category)
    histogramDir = getHistogramDir(lepton_selection, hadTau_selection, hadTau_frWeight)
    lines.append("process.analyze_2los_1tau.histogramDir = cms.string('%s')" % histogramDir)
    lines.append("process.analyze_2los_1tau.era = cms.string('%s')" % era)
    lines.append("process.analyze_2los_1tau.triggers_1e = cms.vstring(%s)" % self.triggers_1e)
    lines.append("process.analyze_2los_1tau.use_triggers_1e = cms.bool(%s)" % ("1e" in triggers))
    lines.append("process.analyze_2los_1tau.triggers_2e = cms.vstring(%s)" % self.triggers_2e)
    lines.append("process.analyze_2los_1tau.use_triggers_2e = cms.bool(%s)" % ("2e" in triggers))
    lines.append("process.analyze_2los_1tau.triggers_1mu = cms.vstring(%s)" % self.triggers_1mu)
    lines.append("process.analyze_2los_1tau.use_triggers_1mu = cms.bool(%s)" % ("1mu" in triggers))
    ...
    lines.append("process.analyze_2los_1tau.hadTauGenMatch = cms.string('%s')" % hadTau_genMatch)
    lines.append("process.analyze_2los_1tau.apply_hadTauGenMatching = cms.bool(%s)" % apply_hadTauGenMatching)
    if hadTau_frWeight == "enabled":
      lines.append("process.analyze_2los_1tau.applyJetToTauFakeRateWeight = cms.bool(True)")
      if era == "2015":
        lines.append("process.analyze_2los_1tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2015.root')")
        ...
        lines.append("process.analyze_2los_1tau.jetToTauFakeRateWeight.lead.graphName = cms.string('%s'" % graphName)
        lines.append("process.analyze_2los_1tau.jetToTauFakeRateWeight.lead.fitFunctionName = cms.string('%s'" % fitFunctionName)
      elif era == "2016":
        lines.append("process.analyze_2los_1tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2016.root')")
      else:
        raise ValueError("Invalid parameter 'era' = %s !!" % era)
    elif hadTau_frWeight == "disabled":
      lines.append("process.analyze_2los_1tau.applyJetToTauFakeRateWeight = cms.bool(False)")
    else:
      raise ValueError("Invalid parameter 'hadTau_frWeight' = %s !!" % hadTau_frWeight)
    if hadTau_selection.find("mcClosure") != -1:
      lines.append("process.analyze_2los_1tau.jetToTauFakeRateWeight.applyFitFunction_lead = cms.bool(False)")
    ...
    lines.append("process.analyze_2los_1tau.lumiScale = cms.double(%f)" % lumi_scale)
    lines.append("process.analyze_2los_1tau.apply_trigger_bits = cms.bool(%s)" % apply_trigger_bits)
  ...
  def createCfg_makePlots_mcClosure(self):
    ...
    lines.append("  ),")
    lines.append("  cms.PSet(")
    lines.append("    histogramName = cms.string('sel/evt/$PROCESS/mvaOutput_2los_1tau_ttbar'),")
  ...
  def create(self):
            ...
            for central_or_shift in self.central_or_shifts:
              for jobId in range(len(self.inputFileIds[sample_name])):
                ...
                self.createCfg_analyze(self.ntupleFiles[key_file], self.histogramFiles[key_file], sample_category, self.era, triggers,
                  self.lepton_selection, hadTau_selection, hadTau_genMatch, self.apply_hadTauGenMatching, hadTau_frWeight, 
      ...
      for hadTau_selection in self.hadTau_selections:
        for hadTau_frWeight in self.hadTau_frWeights:
          ...
          self.cfgFile_addBackgrounds_modified[key] = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_%s_%s_cfg.py" % \
            (self.channel, process_name, hadTau_selection_and_frWeight))
          histogramDir = getHistogramDir(self.lepton_selection, hadTau_selection, hadTau_frWeight, hadTau_charge_selection)
    ...
    for hadTau_selection in self.hadTau_selections:
      for hadTau_frWeight in self.hadTau_frWeights:
        ...
        self.cfgFile_addBackgrounds_modified[key] = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_fakes_mc_%s_cfg.py" % \
          (self.channel, hadTau_selection_and_frWeight))
        histogramDir = getHistogramDir(self.lepton_selection, hadTau_selection, hadTau_frWeight)
        processes_input = []
    ...
    category_signal = "2los_1tau_lepTight_tauTight" 
    category_sideband = "2los_1tau_lepTight_tauFakeable_wFakeRateWeights" 
