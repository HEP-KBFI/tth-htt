...
import tthMVATrainingNtupleSamples_2los_1tau
from tthAnalysis.HiggsToTauTau.jobTools import create_if_not_exists

class mvaTrainingNtupleConfig_2los_1tau(analyzeConfig):
  ...
  def __init__(self, outputDir, executable_analyze, hadTau_selection, central_or_shifts,
    ...
    self.samples = tthMVATrainingNtupleSamples_2los_1tau.samples
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "produceTTHMVATrainingNtuple_2los_1tau_cfg.py")
    self.histogramDir_prep_dcard = "ntuple_2los_1tau_Tight"
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, triggers, lepton_selection, hadTau_selection, 
    ...
    """Create python configuration file for the produceTTHMVATrainingNtuple_2los_1tau executable (analysis code)
    ...
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.process = cms.string('%s')" % sample_category)
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.use_triggers_1e = cms.bool(%s)" % ("1e" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.use_triggers_2e = cms.bool(%s)" % ("2e" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.use_triggers_1mu = cms.bool(%s)" % ("1mu" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.use_triggers_2mu = cms.bool(%s)" % ("2mu" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.use_triggers_1e1mu = cms.bool(%s)" % ("1e1mu" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.leptonSelection = cms.string('%s')" % lepton_selection)
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.hadTauSelection = cms.string('%s')" % hadTau_selection)
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.isMC = cms.bool(%s)" % is_mc)
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.central_or_shift = cms.string('%s')" % central_or_shift)
    lines.append("process.produceTTHMVATrainingNtuple_2los_1tau.lumiScale = cms.double(%f)" % lumi_scale)
