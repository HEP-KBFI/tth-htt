...
class analyzeConfig_2lss_1tau(analyzeConfig):
  ...
  def createCfg_analyze(self, jobOptions):
    ...
    if jobOptions['hadTau_selection'].find("Fakeable") != -1 and jobOptions['applyFakeRateWeights'] in [ "3L", "1tau" ]:
      ...
      if jobOptions['era'] == "2015":
        ...
        raise ValueError("Invalid parameter 'era' = %s !!" % jobOptions['era'])
  ...
  def addToMakefile_make_plots(self, lines_makefile):
    ...
    for idxJob, jobOptions in enumerate(self.jobOptions_make_plots.values()):
      ...
      lines_makefile.append("\t%s %s" % (self.executable_make_plots, jobOptions['cfgFile_modified']))
      lines_makefile.append("")
      ...
      for lepton_and_hadTau_frWeight in self.lepton_and_hadTau_frWeights:          
        ...
        for lepton_charge_selection in self.lepton_charge_selections:
          ...
          for sample_name, sample_info in self.samples.items():
            ...
            # output processes: TT; ...
            key_hadd_stage1 = getKey(process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)
            key_addBackgrounds_job = getKey(process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)
            self.jobOptions_addBackgrounds[key_addBackgrounds_job] = {
              'inputFile' : self.outputFile_hadd_stage1[key_hadd_stage1],
              'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_%s_%s_%s_cfg.py" % \
                (self.channel, process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)),
              'outputFile' : os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_%s_%s_%s.root" % \
                (self.channel, process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)),
              'categories' : [ getHistogramDir(lepton_and_hadTau_selection, lepton_and_hadTau_frWeight, lepton_charge_selection) ],
            ...
            key_hadd_stage1 = getKey(process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)
            key_addBackgrounds_job = getKey("fakes_%s" % process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)
            self.jobOptions_addBackgrounds[key_addBackgrounds_job] = {
              'inputFile' : self.outputFile_hadd_stage1[key_hadd_stage1],
              'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_fakes_%s_%s_%s_cfg.py" % \
                (self.channel, process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)),
              'outputFile' : os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_fakes_%s_%s_%s.root" % \
                (self.channel, process_name, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection)),
              'categories' : [ getHistogramDir(lepton_and_hadTau_selection, lepton_and_hadTau_frWeight, lepton_charge_selection) ],
              'processes_input' : [ "%s%s" % (process_name, genMatch) for genMatch in self.lepton_and_hadTau_genMatches_fakes ],
              'process_output' : "fakes_%s" % process_name
            }
            self.createCfg_addBackgrounds(self.jobOptions_addBackgrounds[key_addBackgrounds_job])
          ...
          key_hadd_stage2 = getKey(lepton_charge_selection)
