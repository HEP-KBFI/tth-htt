...
class analyzeConfig_2lss_1tau(analyzeConfig):
  ...
  """
  def __init__(self, outputDir, executable_analyze, cfgFile_analyze, samples, changeBranchNames,
  ...
  def createCfg_analyze(self, jobOptions):
    ...
    lines.append("process.analyze_2lss_1tau.use_HIP_mitigation_bTag = cms.bool(%s)" % jobOptions['use_HIP_mitigation_bTag'])
  ...
  def create(self):
        ...
        for lepton_charge_selection in self.lepton_charge_selections:
          for chargeSumSelection in self.chargeSumSelections:
            ...
            for sample_name, sample_info in self.samples.items():
              ...
              for central_or_shift in self.central_or_shifts:
                ...
                for jobId in inputFileList.keys():
                  ...
                  self.outputFile_hadd_stage1[key_hadd_stage1] = os.path.join(self.outputDir, DKEY_HIST, "histograms_harvested_stage1_%s_%s_%s_lep%s_sum%s.root" % \
                ...
                for sample_category in sample_categories:
                  ...
                  for genMatch_category in genMatch_categories:
                    ...
                    if genMatch_category == "nonfake":
                      ...
                      cfgFile_modified = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_%s_%s_%s_lep%s_sum%s_cfg.py" % \
                      ...
                      outputFile = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_%s_%s_%s_lep%s_sum%s.root" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      ...
                      cfgFile_modified = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_fakes_%s_%s_%s_lep%s_sum%s_cfg.py" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      outputFile = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_fakes_%s_%s_%s_lep%s_sum%s.root" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      ...
                      cfgFile_modified = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_gentau_%s_%s_%s_lep%s_sum%s_cfg.py" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      outputFile = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_gentau_%s_%s_%s_lep%s_sum%s.root" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      ...
                      cfgFile_modified = os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_faketau_%s_%s_%s_lep%s_sum%s_cfg.py" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
                      outputFile = os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_faketau_%s_%s_%s_lep%s_sum%s.root" % \
                        (self.channel, process_name, sample_category, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))                    
                    if processes_input:
                      ...
                      self.outputFile_hadd_stage1_5[key_hadd_stage1_5] = os.path.join(self.outputDir, DKEY_HIST, "histograms_harvested_stage1_5_%s_%s_lep%s_sum%s.root" % \
            ...
            self.jobOptions_addBackgrounds[key_addBackgrounds_job] = {
              ...
              'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addBackgrounds_%s_fakes_mc_%s_lep%s_sum%s_cfg.py" % \
                (self.channel, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection)),
              'outputFile' : os.path.join(self.outputDir, DKEY_HIST, "addBackgrounds_%s_fakes_mc_%s_lep%s_sum%s.root" % \
                (self.channel, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection)),
              'categories' : [ getHistogramDir(lepton_selection, hadTau_selection, lepton_and_hadTau_frWeight, lepton_charge_selection, chargeSumSelection) ],
              'processes_input' : processes_input,
            ...
            self.outputFile_hadd_stage2[key_hadd_stage2] = os.path.join(self.outputDir, DKEY_HIST, "histograms_harvested_stage2_%s_%s_lep%s_sum%s.root" % \
              (self.channel, lepton_and_hadTau_selection_and_frWeight, lepton_charge_selection, chargeSumSelection))
    ...
    for lepton_charge_selection in self.lepton_charge_selections:
      for chargeSumSelection in self.chargeSumSelections:
        ...
        self.jobOptions_addFakes[key_addFakes_job] = {
          'inputFile' : self.outputFile_hadd_stage1_5[key_hadd_stage1_5],
          'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addBackgroundLeptonFakes_%s_lep%s_sum%s_cfg.py" % \
            (self.channel, lepton_charge_selection, chargeSumSelection)),
          'outputFile' : os.path.join(self.outputDir, DKEY_HIST, "addBackgroundLeptonFakes_%s_lep%s_sum%s.root" % \
            (self.channel, lepton_charge_selection, chargeSumSelection)),
    ...
    for chargeSumSelection in self.chargeSumSelections:
      ...
      self.outputFile_hadd_stage1_6 = os.path.join(self.outputDir, DKEY_HIST, "histograms_harvested_stage1_6_%s_Tight_lepOS_sum%s.root" % \
    ...
    for chargeSumSelection in self.chargeSumSelections:
      ...
      self.jobOptions_addFlips[key_addFlips_job] = {
        ...
        'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addBackgroundLeptonFlips_%s_sum%s_cfg.py" % \
          (self.channel, chargeSumSelection)),
        'outputFile' : os.path.join(self.outputDir, DKEY_HIST, "addBackgroundLeptonFlips_%s_sum%s.root" % \
          (self.channel, chargeSumSelection)),
    ...
    for chargeSumSelection in self.chargeSumSelections:
      ...
      for histogramToFit in self.histograms_to_fit:
        ...
        self.jobOptions_prep_dcard[key_prep_dcard_job] = {
          ...
          'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "prepareDatacards_%s_sum%s_%s_cfg.py" % (self.channel, chargeSumSelection, histogramToFit)),
          'datacardFile' : os.path.join(self.outputDir, DKEY_DCRD, "prepareDatacards_%s_sum%s_%s.root" % (self.channel, chargeSumSelection, histogramToFit)),
        ...
        if chargeSumSelection == "OS" and histogramToFit in [ "mvaDiscr_2lss" ]:
          ...
          self.jobOptions_add_syst_dcard[key_add_syst_dcard_job] = {
            ...
            'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "addSystDatacards_%s_sum%s_%s_cfg.py" % (self.channel, chargeSumSelection, histogramToFit)),
            'outputFile' : os.path.join(self.outputDir, DKEY_DCRD, "addSystDatacards_%s_sum%s_%s.root" % (self.channel, chargeSumSelection, histogramToFit)),
      ...
      for chargeSumSelection in self.chargeSumSelections:
        ...
        self.jobOptions_make_plots[key_makePlots_job] = {
          ...
          'inputFile' : self.outputFile_hadd_stage2[key_hadd_stage2],
          'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "makePlots_%s_sum%s_cfg.py" % (self.channel, chargeSumSelection)),
          'outputFile' : os.path.join(self.outputDir, DKEY_PLOT, self.channel, "makePlots_%s_sum%s.png" % (self.channel, chargeSumSelection)),
          'histogramDir' : (self.histogramDir_prep_dcard % chargeSumSelection),
          'label' : None,
        ...
        if "OS" in self.lepton_charge_selections and chargeSumSelection == "OS":
          ...
          self.jobOptions_make_plots[key_makePlots_job] = {
            'executable' : self.executable_make_plots,
            'inputFile' : self.outputFile_hadd_stage2[key_hadd_stage2],
            'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "makePlots_%s_lepOS_sum%s_cfg.py" % (self.channel, chargeSumSelection)),
            'outputFile' : os.path.join(self.outputDir, DKEY_PLOT, self.channel, "makePlots_%s_lepOS_sum%s.png" % (self.channel, chargeSumSelection)),                       
        ...
        if "Fakeable_mcClosure" in self.lepton_and_hadTau_selections and chargeSumSelection == "OS":
          ...
          self.jobOptions_make_plots[key_makePlots_job] = {
            ...
            'inputFile' : self.outputFile_hadd_stage2[key_hadd_stage2],
            'cfgFile_modified' : os.path.join(self.outputDir, DKEY_CFGS, "makePlots_mcClosure_%s_sum%s_cfg.py" % (self.channel, chargeSumSelection)),
            'outputFile' : os.path.join(self.outputDir, DKEY_PLOT, self.channel, "makePlots_mcClosure_%s_sum%s.png" % (self.channel, chargeSumSelection)),
