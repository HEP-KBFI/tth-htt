...
class analyzeConfig_ttZctrl(analyzeConfig):
  ...
  def __init__(self, configDir, outputDir, executable_analyze, cfgFile_analyze, samples, hadTau_selection, central_or_shifts,
    ...
    self.cfgFile_make_plots = os.path.join(self.workingDir, "makePlots_ttZctrl_cfg.py")    
  ...
  def create(self):
    ...
    if self.is_sbatch:
      ...
      self.createScript_sbatch(self.executable_analyze, self.sbatchFile_analyze, self.jobOptions_analyze, key_input_file = 'ntupleFiles', key_output_file = 'histogramFile')
