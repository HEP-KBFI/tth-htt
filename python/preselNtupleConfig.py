...
class preselNtupleConfig:
    ...
    Args:
        ...
        executable_preselNtuple: Name of the executable that runs the Ntuple production
    ...
    """
    def __init__(self, configDir, outputDir, executable_preselNtuple, channel, samples,
                 era, debug, running_method, rle_directory, version, num_parallel_jobs,
                 pool_id = ''):

        self.configDir = configDir
        self.outputDir = outputDir
        self.executable_preselNtuple = executable_preselNtuple
        ...
        self.dirs = {}
        self.samples = samples
        self.cfgFiles_preselNtuple_modified = {}
        self.logFiles_preselNtuple = {}
        self.sbatchFile_preselNtuple = os.path.join(
          self.configDir, "sbatch_preselNtuple_%s.py" % self.channel)
        self.inputFiles = {}
        self.outputFiles = {}
        self.filesToClean = []
    ...
    def createCfg_preselNtuple(self, *args):
        raise ValueError(
            "Function 'createCfg_preselNtuple' not implemented in derrived class !!")
    ...
    def createScript_sbatch(self):
        ...
        tools_createScript_sbatch(
            sbatch_script_file_name = self.sbatchFile_preselNtuple,
            executable = self.executable_preselNtuple,
            cfg_file_names = self.cfgFiles_preselNtuple_modified,
            input_file_names = self.inputFiles,
            output_file_names = self.outputFiles,
            log_file_names = self.logFiles_preselNtuple,
    ...
    def addToMakefile_preselNtuple(self, lines_makefile):
        """Adds the commands to Makefile that are necessary for running the Ntuple production code
        """
        if self.is_sbatch:
            lines_makefile.append("sbatch_preselNtuple:")
            lines_makefile.append("\t%s %s" % (
                "python", self.sbatchFile_preselNtuple))
            lines_makefile.append("")
        for key_file, output_file in self.outputFiles.items():
            cfg_file_preselNtuple_modified = self.cfgFiles_preselNtuple_modified[key_file]
            if self.is_makefile:
                lines_makefile.append("%s:" % output_file)
                cfg_file = cfg_file_preselNtuple_modified
                log_file = self.logFiles_preselNtuple[key_file]
                lines_makefile.append("\t%s %s &> %s" % (
                    self.executable_preselNtuple, cfg_file, log_file))
                lines_makefile.append("")
            elif self.is_sbatch:
                lines_makefile.append("%s: %s" % (output_file, "sbatch_preselNtuple"))
    ...
    def create(self):
        ...
        for sample_name, sample_info in self.samples.items():
            ...
            logging.info("Creating configuration files to run '%s' for sample %s" % (self.executable_preselNtuple, process_name))  
            ...
            for jobId in inputFileList.keys():
                ...
                self.cfgFiles_preselNtuple_modified[key_file] = os.path.join(self.dirs[key_dir][DKEY_CFGS], "preselNtuple_%s_%s_%i_cfg.py" % \
                  (self.channel, process_name, jobId))
                self.outputFiles[key_file] = os.path.join(self.dirs[key_dir][DKEY_NTUPLES], "%s_%i.root" % \
                  (process_name, jobId))
                self.logFiles_preselNtuple[key_file] = os.path.join(self.dirs[key_dir][DKEY_LOGS], "preselNtuple_%s_%s_%i.log" % \
                  (self.channel, process_name, jobId))
                self.createCfg_preselNtuple(self.inputFiles[key_file], self.outputFiles[key_file], self.era, is_mc, 
                                            self.cfgFiles_preselNtuple_modified[key_file], rle_filename)
                
        if self.is_sbatch:
            logging.info("Creating script for submitting '%s' jobs to batch system" % self.executable_preselNtuple)
            self.createScript_sbatch()

        logging.info("Creating Makefile")
        lines_makefile = []
        self.addToMakefile_preselNtuple(lines_makefile)
