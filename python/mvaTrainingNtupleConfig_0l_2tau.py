...
import tthMVATrainingNtupleSamples_0l_2tau
...
class mvaTrainingNtupleConfig_0l_2tau(analyzeConfig):
  """Configuration metadata needed to run analysis in a single go.
  
  Sets up a folder structure by defining full path names; no directory creation is delegated here.
  
  Args specific to mvaTrainingNtupleConfig_0l_2tau:
  ...
  def __init__(self, outputDir, executable_analyze, hadTau_selections, hadTau_charge_selections, central_or_shifts,
    ...
    self.samples = tthMVATrainingNtupleSamples_0l_2tau.samples
    ...
    self.cfgFile_analyze_original = os.path.join(self.workingDir, "produceTTHMVATrainingNtuple_0l_2tau_cfg.py")
    self.histogramDir_prep_dcard = "ntuple_0l_2tau_OS_Tight"
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, triggers, hadTau_selection, hadTau_genMatch, hadTau_frWeight, hadTau_charge_selection,
    ...
    """Create python configuration file for the produceTTHMVATrainingNtuple_0l_2tau executable (analysis code)
    ...
    Args:
      ...
      central_or_shift: either 'central' or one of the systematic uncertainties defined in $CMSSW_BASE/src/tthAnalysis/HiggsToTauTau/bin/produceTTHMVATrainingNtuple_0l_2tau.cc
    """  
    lines = []
    ...
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.process = cms.string('%s')" % sample_category)
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.use_triggers_1e = cms.bool(%s)" % ("1e" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.use_triggers_1mu = cms.bool(%s)" % ("1mu" in triggers))
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.hadTauChargeSelection = cms.string('%s')" % hadTau_charge_selection)
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.hadTauSelection = cms.string('%s')" % hadTau_selection)
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.hadTauGenMatch = cms.string('%s')" % hadTau_genMatch)
    if hadTau_frWeight == "enabled":
      lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.applyJetToTauFakeRateWeight = cms.bool(True)")
    elif hadTau_frWeight != "disabled":
      raise ValueError("Invalid parameter 'hadTau_frWeight' = %s !!" % hadTau_frWeight) 
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.isMC = cms.bool(%s)" % is_mc)
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.central_or_shift = cms.string('%s')" % central_or_shift)
    lines.append("process.produceTTHMVATrainingNtuple_0l_2tau.lumiScale = cms.double(%f)" % lumi_scale)
