...
class analyzeConfig_1l_2tau(analyzeConfig):
  ...
  def createCfg_analyze(self, inputFiles, outputFile, sample_category, era, triggers,
    ...
    if hadTau_frWeight == "enabled":
      ...
      if era == "2015":
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2015.root')")
        ...
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.lead.graphName = cms.string('%s'" % graphName)
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.lead.fitFunctionName = cms.string('%s'" % fitFunctionName)
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.sublead.graphName = cms.string('%s'" % graphName)
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.sublead.fitFunctionName = cms.string('%s'" % fitFunctionName)
      elif era == "2016":
        lines.append("process.analyze_1l_2tau.jetToTauFakeRateWeight.inputFileName = cms.string('tthAnalysis/HiggsToTauTau/data/FR_tau_2016.root')")
      else:
        raise ValueError("Invalid parameter 'era' = %s !!" % era)
    elif hadTau_frWeight == "disabled":
      lines.append("process.analyze_1l_2tau.applyJetToTauFakeRateWeight = cms.bool(False)")
    else:
